...
class TweetsProcessorDEV {
  ...
  extractTweetData(tweets) {
    for (let i = 0; i < tweets.length; i++) {
      let tweetId = tweets[i].getAttribute('data-tweet-id');
      let tweetTimestamp = new Date(tweets[i].querySelector('time').getAttribute('datetime')).toISOString();
      let tweetText = tweets[i].querySelector('p.timeline-Tweet-text').innerHTML;
  ...
  setClickListenersOnTweets() {
      let shadowDoc = $('#twitter-widget-0')[0].contentDocument,
      ...
      this.extractTweetData(tweets);

      loadButton.onclick = () => {
        setTimeout(() => {
          let updatedTweets = $('#twitter-widget-0')[0].contentDocument.querySelectorAll('div[data-tweet-id]');
