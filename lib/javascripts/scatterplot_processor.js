...
export class ScatterplotProcessor {
  ...
  updateScatterplot(changedData) {
    this.toneKey = currentTone();

    if (changedData) {
      let newDataIsComingIn = changedData.length > this.copyTweetData.length;

      if (newDataIsComingIn) {
        this.copyTweetData = merge([], changedData);
        this.rescaleXAxesAndZoom(changedData);
      }
  ...
  updateCircleGroups() {
    this.dataColumns = d3.select(".tweetCircleGroup")
                         .data(this.tweetsData, d => d.id)
                         .enter()
                         .append("g")
                         .attr("class", "tweetCircleGroup")
