...
export class ScatterplotProcessor {
  ...
  createScatterplot(data) {
    ...
    d3.select(".selectToneDropdown")
      .on("change", this.updateScatterplot.bind(this));
  }
  ...
  setupLegend() {
    let legendContainers = d3.select(".legend")
                             ...
                             .call(this.toggleLegend.bind(this));
    ...
    legendContainer.on("click", function(d, i) {
      ...
      if (this.getAttribute("class").includes("off")) {
        that.tweetsData.forEach(tweet => {
          delete tweet[that.toneKey][d.tone];
        });
      } else {
        that.tweetsData.forEach((tweet, idx) => {
          tweet[that.toneKey][d.tone] = that.copyTweetData[idx][that.toneKey][d.tone];
        });
      }

      that.toneToggleStatus[d.tone] = !that.toneToggleStatus[d.tone];
      that.updateScatterplot(that.tweetsData);
    });
  }
  ...
  updateScatterplot(changedData) {
    ...
    if (changedData) {
      ...
      if (this.newDataIsComingIn) {
        ...
        this.resetLegendBoxes();
      }

      this.tweetsData = changedData;
    }
  ...
  resetLegendBoxes() {
    for (let key in this.toneToggleStatus) {
      this.toneToggleStatus[key] = true;
    }

    $(".legendCategoryContainer").removeClass("off");
  }
