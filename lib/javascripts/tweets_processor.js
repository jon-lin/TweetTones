...
class TweetsProcessor {
  constructor(shadowDoc) {
    ...
    this.tweetsHash = {};
    this.SentimentsProcessor = new SentimentsProcessor(this.tweetsHash, this.loadButton);
  ...
  extractTweetDataAndAddSentimentDataAndClickListeners(i) {
      ...
      this.tweetsHash[id] = {id, timestamp, body, handle: this.twitterHandle};
  ...
  addSentimentData(id, body, i) {
    ...
    $.ajax(`api/tweets/${id}/sentiments`).then(result => {
      if (result.error) {
        ...
        this.tweetsHash['emotion_tone'] = JSON.parse(result.emotion_tone);
        this.tweetsHash['language_tone'] = JSON.parse(result.language_tone);
        this.tweetsHash['social_tone'] = JSON.parse(result.social_tone);
  ...
  // attemptToRetrieveFromDatabase(id) {
  //   $.ajax(`/api/tweets/${id}/sentiments`)
  //     .then()
  // }
  // $.ajax('api/tweets/876558049565188096/sentiments').then(result => result.error || result.emotion_tone)
  ...
  //     timestamp: "HELLO",
  //     body: "BODY HELLO",
  //     emotion_tone: "emotion HELLO",
  //     language_tone: "language HELLO",
  //     social_tone: "social HELLO"
  //   }),
