...
class SentimentsProcessor {
  ...
  styleToolTip(tooltipModel, chartEl) {
    ...
    let absLeftPosition = position.left + tooltipModel.caretX + window.scrollX,
        windowWidth = $(window).width();

    if (absLeftPosition + 200 > windowWidth) {
      console.log('offscreen');
      tooltipEl.style.left = '';
      tooltipEl.style.right = windowWidth - absLeftPosition + 'px';
    } else {
      console.log('onscreen');
      tooltipEl.style.right = '';
      tooltipEl.style.left = absLeftPosition + 'px';
    }
  }
