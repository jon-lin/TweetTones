...
class SentimentsProcessor {
  ...
  chartData(labels, label, data, colors) {
    return {
      ...
      options: {
        title: {
          display: true,
          text: 'Tone Analyzer Data For Tweet Above',
          fontSize: 20
        },
        layout: {
          padding: {
  ...
  lineGraphOptions() {
    ...
    return {
      title: {
        display: true,
        text: 'Emotion Data For All Fetched Tweets Mapped Over Time',
        fontSize: 25
      },
      layout: {
        padding: {
  ...
  createLineGraphs() {
    ...
    $('.barcharts-carousel-container').after('<div class="chartSeparatorLine"></div>');
