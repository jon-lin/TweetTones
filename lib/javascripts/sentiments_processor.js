...
class SentimentsProcessor {
  ...
  createSentimentDataBarCharts(id) {
    ["emotion_tone", "language_tone", "social_tone"].forEach(title => {
      let currentTweetTones = this.tweetsHash[id][title];
      let labels = Object.keys(currentTweetTones);
      let data = Object.values(currentTweetTones);
      let barchartsProcesser = new BarChartsProcessor
      barchartsProcesser.createBarChart(data, labels, title);
    })
  }
