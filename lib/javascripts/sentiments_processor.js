...
class SentimentsProcessor {
  ...
  lineGraphOptions() {
    return {
      ...
      tooltips: {
        callbacks: {
                label: function(tooltipItem, data) { return data.labels[tooltipItem.index] + ' - ' + tooltipItem.yLabel + ' - my text' }
              }
          }
    };
  }
  ...
  createLineGraphs() {
    ...
    //   tooltipTemplate: "<%if (label){%><%=label%>: <%}%><%= 'Hello' %>"
    // }

    // $('#emotion-linechart').on('click', e => {
    //     let activeEl = emotionLineChart.getElementAtEvent(e);
    //     debugger
